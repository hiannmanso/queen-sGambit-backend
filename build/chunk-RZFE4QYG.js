...
// src/repositories/game.respository.ts
async function createNewGame(first_player_id, second_player_id, token) {
...
async function getTable(token, userId) {
  ...
  if (result.white_player == userId) {
    ...
    return __spreadProps(__spreadValues({}, result2), { whitePlayer: true });
  }
  return __spreadProps(__spreadValues({}, result), { whitePlayer: false });
}
async function makeaNewMove(piece, sqToGo) {
...
async function newMoveHistory(piece, sqToGo, token, date) {
...
async function timeToPlay(token, userId) {
...
var game_respository_default = gameRepository;
